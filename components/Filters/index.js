...
class FiltersPanel extends React.Component {
  ...
  constructor(props){
    ...
    this.onWeightChange.bind(this)
    this.onHairChange.bind(this)
    this.onProfessionChange.bind(this)

    //Render Filters
    this.renderAgeFilter.bind(this)
    this.renderHeightFilter.bind(this)
    this.renderWeightFilter.bind(this)
    this.renderHairFilter.bind(this)
    this.renderProfessionFilter.bind(this)
  ...
  onAgeChange = (value) => {
    ...
    dispatch({type: actionTypes.FILTERS_SET_AGE, from: parseInt(value[0]), to: parseInt(value[1])})
  }

  onHeightChange = (value) => {
    const { dispatch } = this.props 
    dispatch({type: actionTypes.FILTERS_SET_HEIGHT, from: parseFloat(value[0]), to: parseFloat(value[1])})
  }

  onWeightChange = (value) => {
    const { dispatch } = this.props 
    dispatch({type: actionTypes.FILTERS_SET_WEIGHT, from: parseFloat(value[0]), to: parseFloat(value[1])})
  }
  ...
  onHairChange = (item) => {
    const { dispatch } = this.props 
    dispatch({type: actionTypes.FILTERS_SET_HAIR, hair: item})
  }
  ...
  renderAgeFilter = () => {
    ...
    return(
      ...
      <Nouislider
        ...
        tooltips onChange={this.onAgeChange}/>
      </div>
  )}
  ...
  renderHeightFilter = () => {
    ...
    return(
      ...
      <Nouislider
        ...
        tooltips onChange={this.onHeightChange}/>
      </div>
  )}
  ...
  renderWeightFilter = () => {
    ...
    return(
      ...
      <Nouislider
        ...
        step={0.1} connect={true}
        format={wNumb({ decimals: 2, thousand: '', suffix: '' })}
        tooltips onChange={this.onWeightChange}/>
      </div>
  )}
      ...
      <div className={classes.optionFilterContainer}>
        <Typography className={classes.optionFilterSubtitle}>Hair Color</Typography>
          {state.filters.hair.options.map(item => {
            return <Typography onClick={() => this.onHairChange(item)} className={classes.optionFilterItem}>{item}</Typography>
          })}
      </div>
  )}
        ...
        <div className={classes.root}>
          ...
          <Typography variant="button" className={classes.results}>
            { (state.inhabitants.display) ? 
                state.inhabitants.display.length :
                state.inhabitants.items.length
            } RESULTS
          </Typography>
...
//Add Styles
FiltersPanel = withStyles(styles)(FiltersPanel)
